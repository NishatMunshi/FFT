...
std::vector<std::complex<double>> FFT(std::vector<std::complex<double>> const &_samples) {
    ...
    auto poly_even = std::vector<std::complex<double>>(n / 2);
    auto poly_odd = std::vector<std::complex<double>>(n / 2);
    for (size_t index = 0; index < n / 2; ++index) {
        poly_even.at(index) = _samples.at(2 * index);
        poly_odd.at(index) = _samples.at(2 * index + 1);
    }

    // find fft of the two halves
    auto const return_even = FFT(poly_even);
    auto const return_odd = FFT(poly_odd);
